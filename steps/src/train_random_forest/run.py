...
def go(args):
    ...
    # (args.trainval_artifact) and save the returned path in train_local_pat
    ...
    # the artifact, and finally use run.log_artifact to log the artifact to
    # the run
...
def get_inference_pipeline(rf_config, max_tfidf_features):
    # Let's handle the categorical features first. Ordinal categorical are
    #  categorical values for which the order is meaningful, for example
    ...
    # production (nor during training). That is not true for neighbourhood_group
    ...
    # last review. First we impute the missing review date with an old date
    # (because there hasn't been a review for a long time), and then we create
    # a new feature from it,
    ...
    processed_features += zero_imputed + ["last_review", "name"]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--trainval_artifact",
        type=str,
        help="Artifact containing the training dataset. It will be split "
    ...
    parser.add_argument(
        "--val_size",
        type=float,
        help="Size of the validation split. Fraction of the dataset, or "
        "number of items",
    )
    ...
    parser.add_argument(
        ...
        " the scikit-learn constructor for RandomForestRegressor.",
        default="{}",
    )
